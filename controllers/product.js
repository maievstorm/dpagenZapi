...
async function getProducts(page = 1) {
    ...
    return db.task(async t => {
        ...
        "FROM public.variant WHERE product_id=product.id) t) AS variants FROM public.product OFFSET $1 LIMIT $2", [offset, config.rowsPerPage]);
        const meta = {page};
        return {
            data,
            meta
...
async function getProductById(id) {
    return db.task(async t => {
        ...
        "prod_price, (SELECT array_to_json(array_agg(row_to_json(t))) " +
        "FROM (SELECT id, variant_name, variant_color, variant_price " +
        "FROM public.variant WHERE product_id=product.id) t) AS variants FROM public.product WHERE id = $1", [id]);
        return {
            data
        }
    });
